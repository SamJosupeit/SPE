...
# Powershell-Script                                                   #
# #####################################################################
# Name:     Functions_ActiveDirectory.ps1                             #
# Funktion: Dieses Script dient der globalen Bereitstellung von       #
#           ActiveDirectory bezogenen Functions                       #
# ################################################################### #
# # Versionsverlauf:                                                # #
# ################################################################### #
...
$global:UsingActiveDirectory = $true

#region Functions

    #region Function Get-ADInfo
    Function Get-ADInfo
    {
        <#
        .SYNOPSIS
            Diese Function erzeugt einen Satz AD-relevanter Daten.

        .DESCRIPTION
            Diese Function erzeugt einen Satz AD-relevanter Daten.

        .EXAMPLE
        (Get-ADInfo).Netbios -> Aufruf des Netbios-Namen
        (Get-ADInfo).FQDN    -> Aufruf des FQDN
        (Get-ADInfo).Domain  -> Aufruf des Active Directory Domain Namens
        (Get-ADInfo).Server  -> Aufruf des Active Directory Primary Server Namens


        .LINK
            https://gallery.technet.microsoft.com/scriptcenter/2ec4405b-6043-4fb1-977c-60c66716a48d

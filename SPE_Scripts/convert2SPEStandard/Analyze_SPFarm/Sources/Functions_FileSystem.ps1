...
#region Functions
	...
	#region Function Get-DirectorySubFolders
	function Get-DirectorySubfolders
	{
        <#
        .SYNOPSIS
        Gibt die in einem Directory beinhalteten SubFolder oder NULL aus
        .DESCRIPTION
        Gibt die in einem Directory beinhalteten SubFolder oder NULL aus
        .EXAMPLE
        $subfolders = Get-DirectorySubfolders $folder
        .PARAMETER $folder
        ...
        process {
			$folders = Get-ChildItem $folderPath | ?{$_.Attributes -eq "Directory"}
			if($folders -ne $null)
			{
				return $folders
			}
	...
	#region Function Get-DirectoryFiles
	function Get-DirectoryFiles
	{
        ...
        process {
			$files = Get-ChildItem $folderPath | ?{$_.Attributes -ne "Directory"}
			if($files -ne $null)
			{
				return $files
			}
    ...
    #region Function ZIP-FileSystemFolder
    Function ZIP-FileSystemFolder
    {
        <#
        ...
        # load assembly
            ...
            if(Test-Path $Target)
            {
                ...
                {
                    Display-TextLine -text "Zipfile $Target existiert bereits. Soll es Überschrieben werden?"
                    $OverwriteZIP = Select-JN
                    if($OverwriteZIP){

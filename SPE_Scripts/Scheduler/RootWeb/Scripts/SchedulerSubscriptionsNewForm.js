...
var NoPCsInfo = '<p style="color: red;">aktuell keine freien PCs zur Verfügung!<br>Anmeldung kann nicht gebucht werden!</p>';
		...
		for(var i = 0; i <= keys.length;i++){
			...
			switch(key){
				...
				case "Teilnehmer":
					...
					break;
				case "FreePCs":
					var selectPcNeeded = document.querySelector('[title="Teilnehmer braucht PC?"]');
					if(value < 1){
						selectPcNeeded.setAttribute("onchange","toggleOnNoFreePC(this);");
						toggleOnNoFreePC(selectPcNeeded);
					}
					break;
				default:
					break;
...
function toggleOnNoFreePC(selectElement){
	var statusSelect = selectElement.checked;
	var parentElement = selectElement.parentElement;
	var infoElement = parentElement.nextElementSibling;
	if(statusSelect){
		infoElement.innerHTML += NoPCsInfo;
		for(var i = 0; i < saveButtons.length; i++){
			var curButton = saveButtons[i];
			curButton.disabled = true;
		}
	} else {
		var oldInfo = infoElement.innerHTML.replace(NoPCsInfo,"");
		infoElement.innerHTML = oldInfo;
		for(var i = 0; i < saveButtons.length; i++){
			var curButton = saveButtons[i];
